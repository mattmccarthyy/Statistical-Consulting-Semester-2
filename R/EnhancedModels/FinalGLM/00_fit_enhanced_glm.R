...
# 4). DrivAge will be a spline w df = 5
# Including in the GLM formula. 
...
# B12 indicator for the interaction
train$B12_only <- ifelse(train$VehBrand == "B12" & train$Region != "R24", 1, 0) # Need to use the R24 handling because other one coefficient is aliased
test$B12_only  <- ifelse(test$VehBrand == "B12" & test$Region != "R24", 1, 0)

# 9). VehGas: binary factor
...
################################################################################
final_glm <- glm(ClaimNb ~ offset(log(Exposure)) + AreaGLM + VehPowerGLM + VehAge_3grp + ns(DrivAge, df = 5) + BM_is50 + BM_above50 + BM_above100 + 
                   VehBrand + VehGas + DensityGLM + Region + VehAge_3grp:VehGas + VehAge_3grp:VehPowerGLM + B12_only:Region, 
...
# Saving everything
################################################################################
# write.csv(final_results, file = "R/EnhancedModels/FinalGLM/FinalGLM_vs_GLM1_comparison.csv")
# saveRDS(final_glm, file = "R/EnhancedModels/FinalGLM/final_glm_(do_not_try_push).rds")
